...
down_revision: Union[str, None] = None  # Base migration - no parent
...
def upgrade() -> None:
    ...
    # Drop existing enum types if they exist (cleanup from previous failed migrations)
    op.execute("DROP TYPE IF EXISTS userrole CASCADE")
    op.execute("DROP TYPE IF EXISTS dossierstatus CASCADE")
    op.execute("DROP TYPE IF EXISTS priority CASCADE")
    op.execute("DROP TYPE IF EXISTS processingstatus CASCADE")
    op.execute("DROP TYPE IF EXISTS fieldstatus CASCADE")
    
    # Create enum types
    user_role_enum = postgresql.ENUM('ADMINISTRATOR', 'VALIDATOR', 'INSTALLER', name='userrole', create_type=False)
    user_role_enum.create(op.get_bind(), checkfirst=False)
    
    dossier_status_enum = postgresql.ENUM('DRAFT', 'SUBMITTED', 'AWAITING_REVIEW', 'IN_REVIEW', 'APPROVED', 'REJECTED', 'ARCHIVED', name='dossierstatus', create_type=False)
    dossier_status_enum.create(op.get_bind(), checkfirst=False)
    
    priority_enum = postgresql.ENUM('LOW', 'NORMAL', 'HIGH', 'URGENT', name='priority', create_type=False)
    priority_enum.create(op.get_bind(), checkfirst=False)
    
    processing_status_enum = postgresql.ENUM('PENDING', 'CLASSIFYING', 'CLASSIFIED', 'EXTRACTING', 'EXTRACTED', 'VALIDATING', 'COMPLETED', 'FAILED', name='processingstatus', create_type=False)
    processing_status_enum.create(op.get_bind(), checkfirst=False)
    
    field_status_enum = postgresql.ENUM('UNREVIEWED', 'CONFIRMED', 'CORRECTED', 'MARKED_WRONG', name='fieldstatus', create_type=False)
    field_status_enum.create(op.get_bind(), checkfirst=False)
    
    # Create base tables first (users and document_types) before any tables that reference them
    ...
    # Add foreign key constraint for document_types.extraction_schema_id after field_schemas is created
    ...
    sa.Column('status', dossier_status_enum, nullable=False),
    ...
    sa.Column('status', field_status_enum, nullable=False),
    ...
    # Note: Old tables (audit_logs, rule_results, rules, submission_files, extracted_data, schemas, submissions) 
    # were already dropped when we cleared the database, so no need to drop them here
